...
describe('applyGroupedAction', () => {
  ...
  it('should handle actions with negative counts', () => {
    ...
    expect(updatedGame).toStrictEqual(mockGame);
    expect(updatedGame.log.length).toBe(1);
  ...
  it('should handle actions with zero count', () => {
    ...
    expect(updatedGame).toStrictEqual(mockGame);
    expect(updatedGame.players[updatedGame.currentPlayerIndex].turn.actions).toBe(1); // Unchanged
    expect(updatedGame.log.length).toBe(1);
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'Error applying grouped action:',
