...
// Configuration
const appName = 'dominion-assistant';
...
if (fs.existsSync(nxBuildDir) && fs.readdirSync(nxBuildDir).length > 0) {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    if (!indexHtml.includes('Content-Security-Policy')) {
      ...
      if (headEnd !== -1) {
        indexHtml = indexHtml.slice(0, headEnd) + 
    ...
    if (bodyEnd !== -1) {
      indexHtml = indexHtml.slice(0, bodyEnd) + 
        `\n  <script>
  // Debug helper
...
if (fs.existsSync(capacitorPluginsDir)) {
  ...
  const buildExtrasPath = path.join(capacitorPluginsDir, 'capacitor-cordova-android-plugins-build-extras.gradle');
  if (!fs.existsSync(buildExtrasPath)) {
    fs.writeFileSync(buildExtrasPath, '// This file is automatically generated.\n// Do not modify - YOUR CHANGES WILL BE ERASED!');
  }
